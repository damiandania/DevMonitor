...
enum NotificationPolicy {
    ...
    /// tells the user, who can stop it from Activity or relaunch it through OwlMonitor. Categorised
    ...
    static func externalProcessDetected(name: String, isBuild: Bool) -> NotificationItem {
        ...
        let body = isBuild
            ? String(localized: "\(name) is building outside Owl Monitor. Run builds via ‘owl-monitor build’ so the server is stopped first.")
            : String(localized: "\(name) is running outside Owl Monitor. Start servers via ‘owl-monitor up’ so they're supervised.")
