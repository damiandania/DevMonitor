...
enum ResourceAdvisor {
    ...
    static func snapshotText(systemCPU: Double, systemMemPercent: Double,
        ...
        for p in procs {
            let tag = p.managedDev ? "[DEV SERVER — managed by Owl Monitor]" : "[foreign]"
    ...
    static func advise(systemCPU: Double, systemMemPercent: Double,
        ...
        let prompt = """
        You are the resource advisor inside **Owl Monitor** on a Mac with \(coreCount) CPU cores.
        ...
        Rules:
        - A process tagged [DEV SERVER — managed by Owl Monitor] can be stopped safely → action
    ...
    static let protectedNames = [
        ...
        "Owl Monitor", "Visual Studio Code", "Cursor", "Terminal", "iTerm",
    ]
    ...
    /// *orphaned* processes (not in Owl Monitor's managed tree) for safe automatic closing and for
    ...
    static func pressureKills(systemCPU: Double, systemMemPercent: Double, systemSwapPercent: Double,
        ...
        never Owl Monitor itself. A [DEV SERVER — managed by Owl Monitor] entry uses "stop_dev_server".
    ...
    static func memoryAdvice(totalMemGB: Double, usedPercent: Double,
        ...
        system daemons, or Owl Monitor itself. (Note: swap can't be flushed directly — macOS shrinks
