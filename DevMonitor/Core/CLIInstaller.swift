...
/// Installs / removes the `dev-monitor` command-line tool by symlinking `~/.local/bin/dev-monitor` at
/// the CLI EMBEDDED in this app bundle (Contents/MacOS/dev-monitor). A symlink — not a copy — so the
...
enum CLIInstaller {
    ...
    static var embeddedCLI: URL? {
        let url = Bundle.main.bundleURL.appendingPathComponent("Contents/MacOS/dev-monitor")
    ...
    static var linkURL: URL { installDir.appendingPathComponent("dev-monitor") }

    /// True when `~/.local/bin/dev-monitor` is our symlink pointing at the embedded CLI. A hand-copied
    /// binary or a symlink to some other dev-monitor reads as NOT installed (so the button offers to
    ...
    static func install() throws {
        guard let embedded = embeddedCLI else {
            throw NSError(domain: "CLIInstaller", code: 1, userInfo: [
                NSLocalizedDescriptionKey: "The dev-monitor CLI isn't bundled in this build of the app."])
        }
